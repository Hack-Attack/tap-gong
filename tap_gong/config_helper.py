...
def extended_config_validation(config):
    ...
    Both start_date(date and time) and end_date(date time) are
    required, check that they are valid.
    """
    try:
        ...
        if not start_date:
            raise BaseException(
                "Missing valid start date in configuration. "
                "Please provide correct start date."
            )
        if not end_date:
            raise BaseException(
                "Missing valid end date in configuration. "
                "Please provide correct end date."
            )
...
def get_stats_dates_from_config(config, retry=False):
    ...
    As per gong API documentation, stats date filter has the following
    requirements:
    ...
    1. If start_date provided in config is greater than or equal to
       the end_date then fromDate will be decreased by one day
    2. In the case of a retry request, we subtract 1 extra day to
       attempt to avoid UTC date conversion issue
    """
    start_date = get_date_time_value_from_config(config, start_date_key)
    end_date = get_date_time_value_from_config(config, end_date_key)
    now = (
